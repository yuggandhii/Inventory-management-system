...
export default function ReceiptCreatePage() {
  ...
  const createMut = useMutation({
    mutationFn: (body) => api.post('/receipts', body),
    onSuccess: (res) => {
      toast.success('Receipt created');
      navigate(`/receipts/${res.data.data.id}`);
    },
    onError: (err) => setError(err.response?.data?.error || 'Failed to create receipt'),
  });
    ...
    <div className="page page-enter" style={{ maxWidth: 900 }}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ marginBottom: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            ...
            <div className="input-group">
              ...
              <input className="input" value={contactName} onChange={(e) => setContactName(e.target.value)} placeholder="Supplier name" />
            </div>
            <div className="input-group">
        ...
        <button type="submit" className="btn btn-primary" disabled={createMut.isPending} style={{ width: '100%' }}>
          {createMut.isPending ? 'Creating...' : 'Create Receipt →'}
        </button>
      </form>
    </div>
  );
